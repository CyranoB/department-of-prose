...
Three scripts ship in this skill's `scripts/` directory and measure different, complementary axes. Run the first two on the input; the third needs a rewrite to compare against, so it runs in the Mode 1 audit.
...
python3 scripts/factcheck.py /tmp/slop-input.txt /tmp/slop-draft.txt   # factual: numbers, quotes, entities, hedges
```

(Save pasted text to `/tmp/slop-input.txt` first. If running from outside the skill directory, use absolute paths.)

`score.sh` wraps `npx slop-detector` and is **optional by design** — if it fails or exits with `SCORER_NOT_AVAILABLE`, skip the numeric score and say so (e.g. `SLOP score: scorer unavailable, qualitative only`). `rhythm.py` and `factcheck.py` are pure Python (no dependencies).
