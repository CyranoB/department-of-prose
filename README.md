...
4. Rewrites the text with a two-pass process: draft, then a fact check and an anti-AI audit that catch what the first pass missed. The fact check is the guardrail on the rewrite: names, numbers, dates, quotations, sources, hedges, and scope limits have to survive the edit, and a pattern that can only be removed by inventing specifics gets left alone or flagged instead.
