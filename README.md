...
4. Rewrites the text with a three-pass audit: draft, a fact check, an anti-AI audit, then a second fact check on the delivered text. A bundled `factcheck.py` (pure Python) computes what survived the edit — numbers, quoted spans, named entities, hedge buckets, and the burstiness delta — so the guardrail is measured rather than asserted. Names, numbers, dates, quotations, sources, hedges, and scope limits have to survive, and a pattern that can only be removed by inventing specifics gets left alone or flagged instead. The fact check is the guardrail on the rewrite: names, numbers, dates, quotations, sources, hedges, and scope limits have to survive the edit, and a pattern that can only be removed by inventing specifics gets left alone or flagged instead.
