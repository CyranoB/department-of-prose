...
- `skills/slop-sense/scripts/rhythm.py` — pure-Python (stdlib only) structural checker for the tells `score.sh` is blind to: burstiness, contraction ratio, aphoristic closers, anaphora, em dashes, curly quotes. These drive perplexity detectors like GPTZero. Reused by `slop-check` via the sibling path `../slop-sense/scripts/rhythm.py` and vendored into the claude.ai bundle.
- `skills/slop-sense/scripts/factcheck.py` — pure-Python (stdlib only) fact-preservation checker. Takes an original and a rewrite and computes what survived: number sets, verbatim quoted spans, capitalised entity sets, hedge buckets (uncertainty / attribution / scope), and the burstiness CV delta. Exists because the editorial fact check was asked to certify its own arithmetic and got it wrong twice (see `evaluation/golden/reviews/` AMD-001 and AMD-002). Compares token sets, not positions, so it narrows the editorial check rather than replacing it. `--strict` gates on hard findings; default is advisory like `rhythm.py`. Vendored into the claude.ai bundle.
...
- The three scripts (`score.sh`, `rhythm.py`, `factcheck.py`) are all optional by design and none may become a hard dependency. `factcheck.py` imports `rhythm.py` for segmentation and burstiness so the two cannot disagree, and falls back to a local copy of that logic if the sibling is absent.
